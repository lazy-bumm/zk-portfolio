$(document).ready(function () {
	...
	function s() {
		$('a[href^="#"]').click(function (e) {
			...
			s.length &&
				(e.preventDefault(), $('html, body').animate({ scrollTop: s.offset().top - 15 }, 300)),
				$('.nav').toggleClass('open');
		});
	}
	function r() {
		$('#full-slide .prev, #full-slide .next').click(function () {
			var e = $(this),
				s = $('.banner').find('.active'),
				r = $('.banner').children().index(s),
				a = $('.banner').children().length;
			e.hasClass('next')
				? a - 1 > r
	...
	function a() {
		$('#three-slide .prev, #three-slide .next').click(function () {
			var e = $(this),
				s = $('.slider').find('.back'),
				r = $('.slider').children().index(s),
				a = $('.slider').find('.current'),
				l = $('.slider').children().index(a),
				n = $('.slider').find('.front'),
				t = $('.slider').children().index(n),
